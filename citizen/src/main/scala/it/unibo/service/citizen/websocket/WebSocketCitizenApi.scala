 ...
 *
 * the endpoint used to create web socket is:
 *  - host/citizens/{id}/state
 *  x FORBIDDEN(403)
 *  x BAD_CONTENT(400)
 *  x NOT_AUTHORIZED(401)
 * the channel supports:
 *  - asynchronous state update push notification:
 *    the server sends to the client a data like:
 *    {
 *      updated: #DATA_SCHEMA
 *    }
 *  - request/respond state updated from the client
 *  the client could update citizen state via thi channel. The operation follows the request/response pattern:
 *  REQUEST PART:
 *  {
 *    id : #incremental id generated by cleint to make each request unique in this channel
 *    value : #DATA_SCHEMA
 *  }
 *  RESPONSE PART:
 *  {
 *    id : #the counterpart of the id generated by the client to link the request to this response
 *    value : #STATUS_SCHEMA
 *  }
 *
 *  #DATA_SCHEMA
 ...
 *  }
 *
 *  #STATUS_SCHEMA: #OK_SCHEMA || #FAIL_SCHEMA
 *
 *  #OK_SCHEMA
 *  {
 *    "status" : "ok"
