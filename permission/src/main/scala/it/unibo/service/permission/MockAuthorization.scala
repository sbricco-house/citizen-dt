...
object MockAuthorization {
  ...
  def roleBased(citizenWritePermission : Set[DataCategory], otherWritePermission : Map[String, Set[DataCategory]], otherReadPermission : Map[String, Set[DataCategory]], allCategories : DataCategory *) = new AuthorizationService {
    override def authorizeRead(who: TokenIdentifier, citizen: String, category: DataCategory): FutureService[DataCategory] = who.identifier match {
      case `citizen` => FutureService.response(category)
      case other => checkRWOther(otherReadPermission, other, category)
    }
    override def authorizeWrite(who: TokenIdentifier, citizen: String, category: DataCategory): FutureService[DataCategory] = who.identifier match {
    ...
    override def authorizedReadCategories(who: TokenIdentifier, citizen: String): FutureService[Seq[DataCategory]] = who.role match {
      case `citizen` => FutureService.response(allCategories)
      case other =>getCategoriesFromOther(otherReadPermission, other)
    }
    override def authorizedWriteCategories(who: TokenIdentifier, citizen: String): FutureService[Seq[DataCategory]] = who.role match {
