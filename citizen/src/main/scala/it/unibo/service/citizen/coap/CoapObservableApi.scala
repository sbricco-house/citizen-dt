...
/**
 * this object allow the creation of a coap server used to observe a citizen state changes.
 * the endpoint supported is:
 *  - OBSERVE host/citizens/{id}/state?data_category=xyz
 *  o RESPONSE(205) : { #DATA_SCHEMA }
 *  x FORBIDDEN(403)
 *  x UNAUTHORIZED(401)
 ...
 *    }
 *  }
 * the request must set "header" authorization in:
 *  2048 -> Bearer token
 */
object CoapObservableApi {
   ...
   * @return a new instance of coap server that wraps a CitizenDigitalTwin.
   */
